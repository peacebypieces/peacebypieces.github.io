...
const timeline = document.getElementById("experienceslist");
    ...
    if (i == exps.length - 1) {
        const expContent = `   
            <li class="mb-10 ms-6">            
                ...
                <h3 class="flex items-center mb-1 text-lg font-semibold text-white"> ${exps[i].name}<span class="bg-blue-100 text-blue-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300 ms-3">Present</span></h3>
                <time class="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">${exps[i].dateRange}</time>
                <p class="mb-4 text-base font-normal text-gray-500 dark:text-gray-400">${exps[i].description}</p>
            </li>
        `
